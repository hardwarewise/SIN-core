...
CMasternode::CMasternode(CService addr, COutPoint outpoint, COutPoint outpointBurnFund, CPubKey pubKeyCollateralAddress, CPubKey pubKeyMasternode, int nProtocolVersionIn) :
    masternode_info_t{ MASTERNODE_ENABLED, nProtocolVersionIn, GetAdjustedTime(),
                       outpoint, outpointBurnFund, addr, pubKeyCollateralAddress, pubKeyMasternode},
    fAllowMixingTx(true)
{}
...
CMasternode::CMasternode(const CMasternodeBroadcast& mnb) :
    masternode_info_t{ mnb.nActiveState, mnb.nProtocolVersion, mnb.sigTime,
                       mnb.vin.prevout, mnb.vinBurnFund.prevout, mnb.addr, mnb.pubKeyCollateralAddress, mnb.pubKeyMasternode,
...
#ifdef ENABLE_WALLET
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string strTxHashBurnFund, std::string strOutputIndexBurnFund, std::string& strErrorRet, CMasternodeBroadcast &mnbRet, bool fOffline)
{
    COutPoint outpoint;
    ...
    uint256 txHashBurnFund = uint256S(strTxHashBurnFund);
    int nOutputIndex = atoi(strOutputIndexBurnFund.c_str());
    COutPoint outpointBurnFund = COutPoint(txHashBurnFund, nOutputIndex);
...
bool CMasternodeBroadcast::Create(const COutPoint& outpoint, const COutPoint& outpointBurnFund, const CService& service, const CKey& keyCollateralAddressNew, const CPubKey& pubKeyCollateralAddressNew, const CKey& keyMasternodeNew, const CPubKey& pubKeyMasternodeNew, std::string &strErrorRet, CMasternodeBroadcast &mnbRet)
{
    ...
    mnbRet = CMasternodeBroadcast(service, outpoint, outpointBurnFund, pubKeyCollateralAddressNew, pubKeyMasternodeNew, PROTOCOL_VERSION);
