...
static void SetupBitcoinTxArgs()
{
    ...
    gArgs.AddArg("sign=SIGHASH-FLAGS", "Add zero or more signatures to transaction. "
        ...
        "See signrawtransaction docs for format of sighash flags, JSON objects.", ArgsManager::ALLOW_ANY, OptionsCategory::COMMANDS);
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " sin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  sin-tx [options] <hex-tx> [commands]  Update hex-encoded sin transaction\n" +
            "or:     sin-tx [options] -create [commands]   Create hex-encoded sin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded sin transaction
