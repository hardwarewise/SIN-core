...
const std::string CBaseChainParams::FINALNET = "final";
...
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const std::string& chain)
{
    if (chain == CBaseChainParams::MAIN)
        return MakeUnique<CBaseChainParams>("", 20971);
    else if (chain == CBaseChainParams::TESTNET)
        return MakeUnique<CBaseChainParams>("testnet3", 20981);
    else if (chain == CBaseChainParams::FINALNET)
        return MakeUnique<CBaseChainParams>("finalnet", 20991);
    else if (chain == CBaseChainParams::REGTEST)
        return MakeUnique<CBaseChainParams>("regtest", 20991);
    else
