...
void musig_api_tests(secp256k1_scratch_space *scratch) {
    ...
    secp256k1_schnorr final_sig_cmp;
    ...
    CHECK(secp256k1_schnorr_verify(vrfy, &final_sig, msg, &combined_pk) == 1);
    ...
    CHECK(ecount == 2);
    {
        secp256k1_schnorr final_sig_tmp = final_sig;
...
void scriptless_atomic_swap(secp256k1_scratch_space *scratch) {
    ...
    secp256k1_schnorr final_sig_a;
    secp256k1_schnorr final_sig_b;
    ...
    CHECK(secp256k1_schnorr_verify(ctx, &final_sig_b, msg32_b, &combined_pk_b) == 1);
    ...
    CHECK(secp256k1_schnorr_verify(ctx, &final_sig_a, msg32_a, &combined_pk_a) == 1);
}
...
void musig_tweak_test_helper(const secp256k1_pubkey* combined_pubkey, const unsigned char *ec_commit_tweak, const unsigned char *sk0, const unsigned char *sk1, const unsigned char *pk_hash) {
    ...
    CHECK(secp256k1_schnorr_verify(ctx, &final_sig, msg, combined_pubkey) == 1);
}
