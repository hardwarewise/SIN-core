...
// NOTE: Unlike wallet RPC (which use SIN values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "SIN is not connected!");
    ...
    if (pindexPrev->nHeight + 1 <= Params().GetConsensus().nDINActivationHeight) {
        ...
        int SINType = 10; // only poll BIG nodes here as they are consensus-critical
        LOCK(infnodeman.cs);
...
static UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    GenerateSINs(fGenerate, nGenProcLimit, Params(), *g_connman);
