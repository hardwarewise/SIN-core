...
{
    return QCoreApplication::translate("sin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. sin_de.qm (shortcut "de" needs to be defined in sin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. sin_de_DE.qm (shortcut "de_DE" needs to be defined in sin.qrc)
...
public:
    ...
    /// Get window identifier of QMainWindow (SINGUI)
...
void BitcoinApplication::setupPlatformStyle()
{
    ...
    platformName = gArgs.GetArg("-uiplatform", SINGUI::DEFAULT_UIPLATFORM);
...
{
    window = new SINGUI(m_node, platformStyle, networkStyle, 0);
...
{
    QMessageBox::critical(0, "Runaway exception", SINGUI::tr("A fatal error occurred. SIN-Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", SINGUI::DEFAULT_UIPLATFORM), true, OptionsCategory::GUI);
}
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data and blocks directory and parse sin.conf
    ...
    if(!masternodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("SIN Core"),
    ...
    // sin: links repeatedly have their payment requests routed to this process:
