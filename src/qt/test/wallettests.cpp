...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<SINAmountField*>("payAmount")->setValue(amount);
...
//
//     src/qt/test/test_sin-qt -platform xcb      # Linux
//     src/qt/test/test_sin-qt -platform windows  # Windows
//     src/qt/test/test_sin-qt -platform cocoa    # macOS
void TestGUI()
{
    ...
    // Amount input
    SINAmountField* amountInput = receiveCoinsDialog.findChild<SINAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: sin:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'test_sin-qt -platform cocoa' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI();
