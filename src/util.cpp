...
fs::path GetDefaultDataDir()
{
    ...
    // Unix: ~/.sin
	fs::path pathSINFolder;
    fs::path pathOldFolder;
#ifdef WIN32
    // Windows
    pathOldFolder = GetSpecialFolderPath(CSIDL_APPDATA) / "SUQA";
	pathSINFolder = GetSpecialFolderPath(CSIDL_APPDATA) / "SIN";

	if (fs::is_directory(pathSINFolder)) return pathSINFolder;
...
#ifdef MAC_OSX
    // Mac
	pathSINFolder = pathRet / "Library/Application Support/SIN";
...
std::string ArgsManager::GetChainName() const
{
    ...
    bool fFinalNet = ArgsManagerHelper::GetNetBoolArg(*this, "-finalnet");
    ...
    if (fFinalNet)
        return CBaseChainParams::FINALNET;
    if (fTestNet)
...
{
    // sorry btc devs, you lost all say due to the way you handled pr#9049
...
{
    boost::filesystem::path pathConfigFile(gArgs.GetArg("-mnconf", MASTERNODE_CONF_FILENAME));
