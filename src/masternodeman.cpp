...
void CMasternodeMan::LocalDiagnostic(int nBlockHeight, int& nSINNODE_1Ret, int& nSINNODE_5Ret, int& nSINNODE_10Ret)
{
    ...
    for (auto& mnpair : mapMasternodes) {
        ...
        //found SINNODE_5 in network
        if (mnpair.second.GetSinType() == CMasternode::SinType::SINNODE_5) {
            nSINNODE_5Ret = 1;
        }
        
        //found SINNODE_10 in network
        if (mnpair.second.GetSinType() == CMasternode::SinType::SINNODE_10) {
            nSINNODE_10Ret = 1;
        }
    }
}
